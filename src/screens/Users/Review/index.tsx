...
import { BookingType,  Inclusion,  PackageType,  UserReviewScreenProps } from 'types/types';
...
const BookingDetails = (props: BookingDetailsProps) => {
  const { review, booking, onBackPress, onTextChange, onRatingChange, onConfirmPress, errorState} = props;
  ...
  return (
    <>
      ...
      <StarRating onRatingChange={onRatingChange} initialRating={review.rating}/>
...
const createReview = async (input: ReviewInputType ) => {
  ...
  try {
    ...
    switch (response.status) {
      ...
      case 401:
        ...
        break;
      case 403:
        setError({error: true, message: 'Forbidden: You do not have permission to perform this action.'});
        break;
      case 404:
        setError({error: true, message: 'Not Found: The requested resource does not exist.'});
        break;
      case 500:
        setError({error: true, message: 'Server Error: Please try again later.'});
        break;
      default:
        setError({error: true, message: `Unexpected error: ${response.status}`});
        break;
    }
  } catch (error) {
    console.log(error)
    setError({error: true, message: 'Network error: Please check your connection.'});
  } finally {
    setLoading(false);
  }
};
  ...
  if(confirmReview){
    return <ReviewConfirmation booking={booking} onConfirm={handSubmit} onCancel={() => setConfirmReview(false)} review={review}/>
  }

  return <BookingDetails review={review} booking={booking} onBackPress={onBackPress} onTextChange={handleTextChange} onRatingChange={handleRatingChange} onConfirmPress={handleConfirm} errorState={error}/>
}
